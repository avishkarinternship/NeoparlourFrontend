...
const KycDocumentCard = ({
  ...
  const fileUrl = document.filePath || document.fileUrl || document.url || document.documentUrl || '';
  const hasFile = !!(document.fileName || document.uploadedAt || fileUrl);
  ...
  return (
    <div className={`rounded-3xl border p-5 sm:p-6 transition-all duration-300 shadow-xs flex flex-col justify-between ${
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] font-semibold text-slate-400 dark:text-zinc-400 mt-0.5 truncate">
                {hasFile ? (
                  <>
                    <span className="font-bold text-slate-700 dark:text-zinc-200">{document.fileName || 'Document_Uploaded'}</span>
          ...
          {/* Status Pill */}
          <span className={`self-start inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase border shrink-0 whitespace-nowrap ${config.badgeClass}`}>
            ...
            <div className="pt-1 pl-6 flex flex-wrap items-center gap-3">
              ...
              {fileUrl && (
                <a
                  ...
                  className="text-[11px] font-bold text-slate-600 dark:text-zinc-300 flex items-center gap-1 hover:underline cursor-pointer"
                >
                  <Eye className="w-3.5 h-3.5" /> View Previous
                </a>
              )}
            </div>
          </div>
        ...
        {/* PENDING INFO */}
        {effectiveStatus === 'PENDING' && (
          <div className="mt-3 p-3.5 rounded-2xl bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/60 dark:border-amber-900/30 text-xs text-amber-700 dark:text-amber-400 font-semibold flex flex-wrap items-center justify-between gap-2">
            <span>Your document is under Admin review.</span>
            <div className="flex items-center gap-3">
              {fileUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[11px] font-bold text-amber-800 dark:text-amber-300 flex items-center gap-1 hover:underline cursor-pointer"
                >
              ...
              <button
                onClick={() => onOpenUploadModal(docType, label)}
                className="text-[11px] font-bold text-amber-800 dark:text-amber-300 underline hover:text-amber-900 cursor-pointer"
              >
                Replace File
              </button>
            </div>
        ...
        {/* APPROVED INFO */}
        {effectiveStatus === 'APPROVED' && (
          <div className="mt-3 p-3.5 rounded-2xl bg-emerald-50/60 dark:bg-emerald-950/20 border border-emerald-200/60 dark:border-emerald-900/30 text-xs text-emerald-700 dark:text-emerald-400 font-semibold flex flex-wrap items-center justify-between gap-2">
