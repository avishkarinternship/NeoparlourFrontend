...
const MyOrders = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    switch (s) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 bg-neutral-100 text-neutral-700 text-[10px] px-2.5 py-1 rounded-full font-black uppercase tracking-wider border border-neutral-200">
            {status}
          </span>
        );
    }
      ...
      <main className="max-w-4xl w-full mx-auto px-4 md:px-8 py-12 flex-1">
        ...
        <div className="mb-8 space-y-2">
          <div className="flex items-center space-x-1.5 text-[11px] text-gray-400 font-medium">
            ...
            <span>&gt;</span>
            <span className="text-gray-600 font-semibold">My Orders</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-black uppercase tracking-wider text-[#131313]">My Orders</h1>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-100 rounded-3xl p-12 text-center shadow-xs flex flex-col items-center justify-center max-w-md mx-auto space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-base font-extrabold text-neutral-900">No Orders Yet</h3>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div 
                key={order.id} 
                className="bg-white border border-gray-100 rounded-3xl p-6 md:p-8 shadow-xs hover:shadow-md transition-shadow duration-300 space-y-6"
              >
                {/* Order Header info */}
                ...
                <div className="space-y-4">
                  {order.items && order.items.map((item) => (
                    <div key={item.id} className="flex gap-4 items-center">
                      <div className="w-14 h-14 bg-neutral-50 border border-neutral-100 rounded-2xl flex items-center justify-center p-1 flex-shrink-0">
                      ...
                      <div className="text-right flex-shrink-0">
                        <span className="text-xs bg-neutral-100 text-neutral-700 px-2 py-0.5 rounded-full font-black font-sans">
                          Qty: {item.quantity}
                        </span>
                      </div>
                    </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-neutral-50 bg-neutral-50/50 -mx-6 md:-mx-8 px-6 md:px-8 -mb-6 md:-mb-8 rounded-b-3xl">
                  <div className="text-xs font-semibold text-neutral-500 font-sans leading-relaxed">
                    Collection Venue: <span className="text-neutral-800 font-bold">{order.salonName || `Salon ID #${order.salonId}`}</span>
